...
示例中的预设名称仅用于说明配置结构；当前提供 Panda、Piper、X5、UR5＋WSG、xArm6＋Robotiq、OpenArm、YAM 双臂部署，关节运动和运动学验收结果见[本体文档](embodiments.md)，具体任务的安装布局和可达性仍需验证。专家将 `manipulator`、`support` 等操作角色绑定到 `left` / `right`；绑定可由配置指定或根据可达性选择，解析结果写入 Episode 元数据。任务声明角色所需能力和协作关系。
...
- 后续本体型号，以及三个任务与对象资产来源；当前七类本体的模型及来源已确定，见[本体文档](embodiments.md)。
