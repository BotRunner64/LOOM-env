...
状态：架构设计与分阶段计划。阶段 A 的配置、Runner、任务检查与离线数据基础已实现并通过自动化测试；已增加六类双臂本体共用接口、真实物理运动诊断、逐帧 FK 检查和视频导出，双 Panda 已接入 ManagerBasedEnv、抓取放置专家和物理重放。详见 [当前实现](implementation.md)。

更新日期：2026-09-10。实际安装版本与验证结果见 [环境文档](environment.md)。
...
示例中的预设名称仅用于说明配置结构；当前提供 Panda、Piper、X5、UR5＋WSG、xArm6＋Robotiq、YAM 双臂部署，关节运动和运动学验收结果见[本体文档](embodiments.md)，具体任务的安装布局和可达性仍需验证。专家将 `manipulator`、`support` 等操作角色绑定到 `left` / `right`；绑定可由配置指定或根据可达性选择，解析结果写入 Episode 元数据。任务声明角色所需能力和协作关系。
...
正式场景采用有固定来源的桌台、房间和物体网格，并保留原资产材质。已接入的 ManiSkill 木桌、RoboDojo 积木和收纳篮，以及资产验收与扩展步骤见 [场景资产接入](scene-assets.md)。碰撞表示按物理与规划需要验证，容器内腔等任务相关几何必须保留。
...
当前已接入 Piper、X5、UR5＋WSG、xArm6＋Robotiq、Panda 和 YAM，覆盖双 6／7 自由度及不同夹爪结构，提供统一关节控制、状态记录与运动学检查。双 Panda 已贯通桌面刚体抓取放置、cuRobo 规划与物理重放，并支持多方块、多容器场景中的目标选择和位置交叉配置；部署中的前视和双腕相机由采集与运动诊断共用。物体尺寸与外观、采样范围和容器位置归 scene，task 定义目标角色与判据，collection 绑定具体对象并生成指令。两臂都要通过关节／TCP 映射、另一臂保持、臂间碰撞和共同时间轴检查；模型最终动作空间仍按研究需求选择。验收至少覆盖一次成功、一次任务失败或超时，以及一次写入中断：有 T 个动作的有效轨迹应保存 T+1 个控制时刻观测；最后一帧来自终止场景；多频率图像通过时间戳及有效性关联。
...
- 后续本体型号，以及三个任务与对象资产来源；当前六类本体的模型及来源已确定，见[本体文档](embodiments.md)。
...
- 已安装版本组合下的协同规划、臂间碰撞和完整任务控制验证；基础本体运动与 FK 已通过，环境检查结果见 [环境文档](environment.md)。
