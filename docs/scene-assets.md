...
RoboDojo USDZ 按字节复制后直接引用，保留刚体层级、凸分解／SDF 和已写入源文件的物理配置。源元数据与 USD 可能不同：例如纸盒 JSON 标注 0.2 kg，但 USD 未写入质量，当前 PhysX 计算约 0.567 kg；健康检查的 `body_mass_kg` 报告实际运行值。加载器不覆盖这些参数。资产目录的动态标记用于校验，场景配置不能把动态源模型改成静态。准备时逐项比较源 USDZ 与缓存引用后的物理 schema 和属性。
...
每份缓存保存来源信息和完整文件哈希；加载时检查文件是否齐全、内容是否与缓存清单一致。资产定义或准备代码更新后仍可使用已有缓存，需要应用更新时手动重新准备对应资产。每个 Episode 保存实际资产版本、稳定后的物理初态和展开配置。
