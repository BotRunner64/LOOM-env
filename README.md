...
- **专家执行：** 七个示范流程统一组合反馈动作，旧的逐 expert 阶段机已删除；[架构与运行验证](docs/implementation.md#共享动作重构)。

- **关节物体：** 固定底座笔记本开盖与半合盖共用 Panda 抓持、关节运动和释放流程，已覆盖目标开角与左右臂变体，并完成代表案例物理重放；[运行、证据与限制](docs/implementation.md#固定底座笔记本开盖)。
